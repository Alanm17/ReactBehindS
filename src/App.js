...
function Tabbed({ content }) {
  ...
  return (
    <div>
      <div className="tabs">
        {Array.from({length:4}, (_,i) => <Tab num={i} activeTab={activeTab} onClick={setActiveTab} />)}
      ...
      {activeTab <= 2 ? (
        <TabContent item={content.at(activeTab)} key={content.at(activeTab).summary} />// at() = [] ex: content[activeTab]
      ) : (
        <DifferentContent />
      )}
    </div>
  ...
  return (
    <div className="tab-content">
      ...
      <div className="tab-undo">
        ...
        <button onClick={lateReset}>Undo in 2s</button>
      </div>
    </div>
  );
}
